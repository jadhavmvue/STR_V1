...
{
    public partial class Form28_2 : Form
    {
        ...
        void gridfill()
        {
            try
            {
                ...
                GlobalClass.adap = new MySqlDataAdapter("SELECT mbatch as `Batch No`, sleeperno as `Sleeper No`, DATE_FORMAT(`date`, '%Y-%m-%d %T') as `Date`,castdate as `Cast Date`, top as `Top`, bottom as `Bottom`, mr1 as `MR1`, mr2 as `MR2`, mf1 as `MF1`, mf2 as `MF2` FROM tbldata_28_2 where mbatch = '" + batch + "' ;", myConn7);
