...
{
    public partial class Form29_2 : Form
    {
        ...
        void gridfill()
        {
            try
            {
                ...
                GlobalClass.adap = new MySqlDataAdapter("SELECT mbatch as `Batch No`, cubeno as `Cube No`, initialwt as `Initial WT`, castdate as `Cast Date`, ageindays as `Age IN Days`, `date` as `Date`,  laodinkn as `Laod in kn`, strength as `Strength` FROM tbldata_29_2 where mbatch = '" + batch + "' ;", myConn7);
